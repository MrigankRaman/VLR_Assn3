...
class MultiHeadAttentionLayer(AttentionLayer):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        dot_product = torch.matmul(query, key.transpose(2,3))/math.sqrt(self.embed_dim/H)
        ...
        if attn_mask is not None:
            ...
            # Think about what inputs make softmax 0.
            additive_mask = -1e9 * (1-attn_mask)
            additive_mask = additive_mask.cuda()
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        output = x + self.encoding(torch.arange(S).cuda())
...
class TransformerDecoder(nn.Module):
    ...
    def get_causal_mask(self, _len):
        ...
        mask = torch.tril(torch.ones(_len, _len), diagonal=0).long()
        return mask
