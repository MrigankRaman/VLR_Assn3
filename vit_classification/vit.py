...
class ViT(nn.Module):
    ...
    def patchify(self, images):
        ...
        patches = images.unfold(1, self.patch_dim, self.patch_dim).unfold(2, self.patch_dim, self.patch_dim)
        patches = patches.reshape(patches.shape[0], patches.shape[1]*patches.shape[2] , patches.shape[3], patches.shape[4], patches.shape[5])
    ...
    def forward(self, images):
        ...
        cls_token = self.cls_token.expand(patches_embedded.shape[0], -1, -1)
        output = torch.cat((cls_token, patches_embedded), dim=1)
        output = self.positional_encoding(patches_embedded)
        mask = torch.ones((self.num_patches, self.num_patches), device=self.device).long()
